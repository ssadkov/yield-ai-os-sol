...
- Для draft PR executor whitelist создана отдельная Preview-ветка `codex/yield-ai-v2-executor-whitelist`. Первое Vercel развёртывание упало при сборке: branch-scoped `NEXT_PUBLIC_PROGRAM_ID` от старого Preview не наследуется, а общий Preview ID не совпадал с IDL `yie1…`. В новую ветку через Vercel API добавлены семь Preview-only переменных из таблицы «RPC для закрытого Preview», включая серверный sensitive Helius URL из локального user environment. Ключ в Git и логи не попадал. Сборку и runtime проверить повторно; эта ветка не является Production и не открывает on-chain новый интерфейс до upgrade.
