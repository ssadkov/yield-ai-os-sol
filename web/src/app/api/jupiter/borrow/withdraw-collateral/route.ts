...
import { runJupiterBorrowCollateralWithdrawJob } from "@/server/agent/runRebalance";
  ...
  try {
    ...
    if (!JUPITER_XSTOCKS_USDC_MARKETS.some((market) => market.vaultId === vaultId)) {
      return NextResponse.json(
        { status: "error", error: "vaultId is not enabled for xStocks / USDC collateral withdraw" },
        { status: 400 },
      );
    }
    ...
    const result = await runJupiterBorrowCollateralWithdrawJob({
      ownerPubkey,
      vaultId,
    ...
    console.error("[jupiter-borrow-withdraw-collateral] failed", {
      message,
