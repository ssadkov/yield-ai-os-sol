...
Unsigned Mainnet-симуляция `withdraw(100000)` для этого Safe (0.1 USDC из 2 USDC, owner как fee payer, `sigVerify=false`) вернула `err=null`, лог `Instruction: Withdraw`, `11643` CU. Ни подписи, ни on-chain вывода не было; живой вывод подтверждается отдельно владельцем в Preview.

## Ручной вход в Kamino владельцем

Кнопки в `/v2/safe` строят два owner-signed пути: из уже свободного USDC Safe или из USDC кошелька через `deposit` в Safe и затем `kamino_deposit`. В обоих случаях временно ставится 100% Kamino allocation, выполняется вход и в **той же атомарной транзакции** восстанавливается прежний allocation. Wallet → Safe → Kamino не использует executor и не оставляет промежуточный депозит при ошибке Kamino. Shares остаются в Safe; shares ATA при необходимости создаётся за счёт SOL владельца.

Read-only Mainnet-симуляции с `FuDC…` и 1 USDC: Safe → Kamino — `err=null`, `148570 CU`, 648 байт; wallet → Safe → Kamino — `err=null`, `160147 CU`, 705 байт. Попытка с 0.1 USDC дошла до Kamino, но была отвергнута `DepositAmountBelowMinimum` (`7026`); поэтому интерфейс консервативно требует не менее 1 USDC для текущего vault. Это **не реальные депозиты** и не доказательство выхода. Баланс кошелька владельца был проверен для симуляции; точное значение в документации не хранится. Подробности и план executor — в [Yield AI v2 executor](yield-ai-v2-executor.md).

PR #18 с whitelist и частичным выводом был смержен в ветку `codex/yield-ai-v2-mainnet` во время этой работы. Новые кнопки и план executor находятся отдельно в draft [PR #19](https://github.com/ssadkov/yield-ai-os-sol/pull/19) (`codex/yield-ai-v2-owner-kamino`). Для его защищённого Preview созданы семь branch-scoped переменных с Mainnet `yie1…`, каноническим USDC и server-only `sensitive` Helius RPC; значения ключей не записаны в Git. После каждой пересборки нужно проверить Mainnet genesis в same-origin RPC и фактический program ID перед подписью. Production не переключался.
