...
- Для draft PR [#18](https://github.com/ssadkov/yield-ai-os-sol/pull/18) создана отдельная Preview-ветка `codex/yield-ai-v2-executor-whitelist`. Её семь branch-scoped Preview переменных указывают на Mainnet `yie1…` и канонический USDC; Helius URL — серверный sensitive. Vercel build прошёл, Preview защищён авторизацией; runtime и пользовательская подпись требуют отдельной проверки. Production не переключён. Старый Preview PR #17 использует старый IDL и не подходит для нового `initialize`.
