...
- Контракт уже принимает один `agent: Pubkey` в owner-signed `initialize`; владелец может заменить или отозвать его через `set_agent`. Sponsored `create_safe_for` всегда создаёт Safe без агента. Текущий UI передаёт `Pubkey::default()`, не предлагает назначение и не запускает автоматический executor. Для текущего пилота все операции подписывает владелец.
- Агент может подписывать только типизированные `kamino_deposit` и `kamino_withdraw` в пределах заданной владельцем allocation. Обычный вывод средств и generic CPI требуют владельца. `allowed_programs` — список программ для owner-only CPI, не whitelist исполнителей. Глобального admin-managed whitelist executor-адресов в развёрнутой версии нет. Для такого whitelist потребуется upgrade программы; отдельный registry PDA позволит не менять размер существующего config, но owner-signed и agent-signed пути должны проверять одобрение, включая отзыв после назначения.
