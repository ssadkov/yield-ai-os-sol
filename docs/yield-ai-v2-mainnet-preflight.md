...
- Админская страница `/v2/admin/executors` позволяет создать и обновить список через кошелёк, чей адрес совпадает с `Config.admin`. Если текущий админский key `8xwj…` доступен только в WSL, для первой транзакции понадобится локальный подписанный скрипт или подключение этого кошелька к браузеру; приватный ключ в UI или Git не помещать. Пользователь выбрал **новый отдельный публичный executor address**; его значение ещё ожидается. Существующий agent key `B1XHm3F9c6fnSRtZaEkuVW2iEnBjLMqk3Dd3NqSUtUE4` не назначать по умолчанию.
...
- Порядок активации: получить только публичный новый executor address; read-only сверить ProgramData, upgrade authority, баланс и стоимость; расширить и обновить программу после отдельного разрешения; сверить байткод; админом инициализировать registry с новым key в approved и default; прочитать PDA; проверить закрытый Preview; затем проводить малый owner-signed цикл Mainnet USDC. До upgrade и registry init нельзя публиковать новый UI вместо действующего Preview: он намеренно блокирует Safe creation.
