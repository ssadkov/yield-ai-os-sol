...
import { runJupiterBorrowUsdcBorrowJob } from "@/server/agent/runRebalance";
  ...
  try {
    ...
    if (!JUPITER_XSTOCKS_USDC_MARKETS.some((market) => market.vaultId === vaultId)) {
      return NextResponse.json(
        { status: "error", error: "vaultId is not enabled for xStocks / USDC borrow" },
        { status: 400 },
      );
    }
    ...
    const result = await runJupiterBorrowUsdcBorrowJob({
      ownerPubkey,
      vaultId,
      amountRaw,
    ...
    console.error("[jupiter-borrow-usdc] failed", {
      message,
